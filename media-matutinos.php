...
<head>
    ...
    <title>Cultos Matutinos | IACyM Lima cercado</title>
        ...
        <div class="container__rsc">
            <div class="content__rsc">
                <div class="heading__rsc">
                    <h1 class="elementor-fn-jost-xbl">CULTOS MATUTINOS</h1>
                </div>
                <div class="container-card__rsc">
                
                    <?php 
                        if(!$_GET) {
                            header('Location:recursos-matutinos.php?pagina=1');
                        }
                        ...
                        $sql_dominicales = 'SELECT * FROM dominicales ORDER BY id DESC LIMIT :inia, :domia';
                        $sentencia_dominicales = $pdo->prepare($sql_dominicales);
                        $sentencia_dominicales->bindParam(':inia', $iniciar, PDO::PARAM_INT);
                        $sentencia_dominicales->bindParam(':domia', $dominical_x_pagina, PDO::PARAM_INT);
                ...
                <nav class="nav-pagination">
                        <ul class="list-page-item">
                            <li class="page-item <?php echo $_GET['paginas']<=1? '.disable' : '' ?>">
                                <a class="page-link" href="recursos-matutinos.php?pagina=<?php echo $_GET['pagina']-1 ?>"><</a>
                            </li>
                            
                            <?php for($i=0; $i<$paginas; $i++): ?>
                                <li class="page-item <?php echo $_GET['paginas']==$i+1 ? '.active' : '' ?>"><a class="page-link" href="recursos-matutinos.php?pagina=<?php echo $i+1 ?>"><?php echo $i+1 ?></a></li>
                            <?php endfor ?>
